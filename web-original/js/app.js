...
class AppState {
    ...
    initializeStorage() {
        return {
            ...
            lastLevel: APP_CONFIG.DEFAULT_LEVEL,
            lastTopic: '',
            levelProgress: {
...
const StorageManager = {
    ...
    restoreFromStorage() {
        const storage = appState.storage;
        if (storage.lastLevel) {
            appState.currentLevel = storage.lastLevel;
        }
        if (storage.lastTopic) {
            appState.currentTopic = storage.lastTopic;
        }
        if (storage.lastMode) {
    ...
    saveProgress() {
        ...
        appState.storage.lastLevel = appState.currentLevel;
        appState.storage.lastTopic = appState.currentTopic;
        this.save();
    }
};
...
const WordManager = {
    async load() {
        try {
            // 优先使用已嵌入的 wordData（如果存在）
            if (typeof wordData !== 'undefined' && wordData && Array.isArray(wordData.words)) {
                appState.words = wordData.words;
                console.log(`✅ 从嵌入数据加载了${appState.words.length}个单词`);
            } else {
                // 回退到 fetch 请求
                const res = await fetch('data/words.json');
                const data = await res.json();
                appState.words = Array.isArray(data.words) ? data.words : [];
                console.log(`✅ 从fetch加载了${appState.words.length}个单词`);
            }
...
// 语音合成服务
const SpeechService = {
    // 检查浏览器是否真正支持语音合成（有可用的英文 voice）
    _hasCheckedVoices: false,
    ...
    checkVoices() {
        if (this._hasCheckedVoices) return this._hasEnglishVoice;
        const voices = window.speechSynthesis?.getVoices() || [];
        this._hasEnglishVoice = voices.some(v => v.lang.startsWith('en'));
        if (voices.length > 0) this._hasCheckedVoices = true;
        return this._hasEnglishVoice;
    },

    // 有道词典 TTS fallback（国内可访问）
    speakWithAudio(text, onStart, onEnd, onError) {
        return new Promise((resolve, reject) => {
            ...
            audio.onerror = (e) => {
                const error = new Error('语音播放失败，请检查网络连接');
                this.handleError(error, onError);
                reject(error);
            };
            audio.play().catch(err => {
                const error = new Error('语音播放失败：' + err.message);
                this.handleError(error, onError);
                reject(error);
            });
        });
    ...
    speak(text, options = {}, onStart = null, onEnd = null, onError = null) {
        // 优先用 Web Speech API，不可用时 fallback 到有道 TTS
        if (!this.isSupported() || !this.checkVoices()) {
            return this.speakWithAudio(text, onStart, onEnd, onError);
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                utterance.onerror = (event) => {
                    console.warn('Web Speech API failed, falling back to Youdao TTS');
                    this.speakWithAudio(text, onStart, onEnd, onError).then(resolve).catch(reject);
                };

                window.speechSynthesis.speak(utterance);

            } catch (error) {
                console.warn('Speech service error, falling back to Youdao TTS');
                this.speakWithAudio(text, onStart, onEnd, onError).then(resolve).catch(reject);
            }
        });
    },
    ...
    isPending() {
        return window.speechSynthesis?.pending || false;
    },
...
document.addEventListener('DOMContentLoaded', async () => {
    // 预加载语音列表（某些浏览器 getVoices 是异步的）
    if (window.speechSynthesis) {
        window.speechSynthesis.getVoices();
        window.speechSynthesis.onvoiceschanged = () => {
            SpeechService._hasCheckedVoices = false;
            SpeechService.checkVoices();
        };
    }
    ...
    if (appState.storage.lastMode) {
        ...
        if (appState.currentMode === 'level') {
            // 恢复 level 按钮的 active 状态
            const levelBtns = DOM.getAll('.level-btn');
            levelBtns.forEach(b => {
                b.classList.remove('active');
                if (b.textContent.trim() === appState.currentLevel) {
