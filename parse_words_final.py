...
# -*- coding: utf-8 -*-
"""
解析KET单词文档，生成words.json - 最终版
基于分析：整个段落的单词都是同一个颜色级别
"""
...
def parse_docx():
    ...
    for para_idx, para in enumerate(doc.paragraphs):
        ...
        # 解析单词列表
        if current_topic and para.runs:
            # 确定整个段落的级别（基于第一个有文本的run）
            level = '黑1'  # 默认级别
            for run in para.runs:
                if run.text.strip():
                    if run.font.color and run.font.color.rgb:
                        color = str(run.font.color.rgb)
                        if color in ['0000FF', '0070C0', '4874CB']:
                            level = '蓝2'
                        elif color == 'C00000':
                            level = '红3'
                    break
...
def generate_phonetic(word):
    """生成简化的音标"""
    # 这里使用简化的占位符，实际项目可以使用专门的音标库
    return f"/{word.lower()}/"

def main():
    """主函数"""
    print("开始解析Word文档（最终版）...")
    ...
    for level in ['黑1', '蓝2', '红3']:
        ...
        for w in level_words:
            print(f"    {w['word']} - {w['meaning']} ({w['topic']})")
